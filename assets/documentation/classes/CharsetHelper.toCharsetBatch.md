...
> **Note**: This method is optimized for homogeneous arrays where all items have the same encoding. For mixed encodings, use `toCharset()` on individual items.

> **Note**: When using AUTO detection, the encoding is detected from the first non-empty string in the array. Empty strings and non-string values are skipped during detection.
